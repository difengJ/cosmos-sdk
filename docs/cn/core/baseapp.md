...
`BaseApp` 是负责SDK应用的核心功能的基础类型，具体是：
...
- 不同的[状态](#states)，基于区块链api接口发送的消息，状态机可以有不同的

The goal of `BaseApp` is to provide the fundamental layer of an SDK applicationthat developers can easily extend to build their own custom application. Usually,
developers will create a custom type for their application, like so:

使用`BaseApp`是为了可以提供一个基本的应用层，开发者可以在这基础上去搭建自己的应用。一般的，开发者可以通过以下方式来创建自己的应用：
...
在原有的`BaseApp`基础上拓展，就可以使用原有`BaseApp`的方法，这样允许开发者专心构造自己的想要的模块，而不用操心去实现ABCI接口，服务路由和状态管理逻辑。

## Type Definition
## 类型定义

The `BaseApp` type holds many important parameters for any Cosmos SDK based application.
`BaseApp` 类型含有很多重要的参数，可以提供给基于Cosmos SDK开发的应用。
...
- [`CommitMultiStore`](./store.md#commitmultistore): 这是整个应用的主要储存，他里面是处理完每个区块之后的规范化状态。这个储存**不是**缓存，所以不是用来更新未被确认的状态`CommitMultiStore`是一个多层次的储存。每个模块使用一个或者多个
`KVStores`用来保存状态。
...
- [`Msg` Service Router](#msg-service-router): `msgServiceRouter` 分配 `Msg` 到对应的模块。这个提到的服务消息是指那些为了改变状态的需要被处理的交易，并不是应用和底层共识引擎的ABCI消息
...
- [gRPC Query Router](#grpc-query-router): `grpcQueryRouter` 分配gRPC查询到对应的模块。这些
...
- [`TxDecoder`](https://godoc.org/github.com/cosmos/cosmos-sdk/types#TxDecoder)用来对原始transcation字节解码

- [`ParamStore`](#paramstore): The parameter store used to get and set application consensus parameters.
- [`ParamStore`](#paramstore)用于获得和设定应用共识参数。
...
- [`AnteHandler`](#antehandler): 这个处理签名验证，费用支付和其他消息执行前检查，这个是在[`CheckTx/RecheckTx`](#checktx) and [`DeliverTx`](#delivertx)期间执行。
...
- `deliverState`: 这个状态会在`DeliverTx`](#delivertx)中更新，在[`Commit`](#commit)中设置为`nil`，在BeginBlock中会重新初始化。
...
`BaseApp`的构造函数很容易理解，值得注意的是可以提供额外[`options`](https://github.com/cosmos/cosmos-sdk/blob/v0.40.0-rc3/baseapp/options.go)。这些`options`会被依次执行，主要都是一些`setter`方法，比如`SetPruning()`来设定pruning选项，`SetMinGasPrices()`来设定节点的最小gas价格。

Naturally, developers can add additional `options` based on their application's needs.
开发者可以根据自己的需要来添加这些额外的`options`参数。
